...
//隐藏各种菜单
    function Hidden_menu() {
        $('#computer_r_menu').css('display', 'none');/*这个是计算机内右键菜单*/
        ...
        $('#comSelet_menu').css('display', 'none');
        return "隐藏菜单成功";
    }
...
// 这3个APP，及Windows的开始菜单被单击时，隐藏所有右键菜单
...
// 是不是重复了
// ANSWER：可是，no。如果隐藏了这个那么点击计算机不会弹出窗口
function Hidden_menu() {
    $('#computer_r_menu').css('display', 'none');
    $('#recycle_D_menu').css('display', 'none');
    $("#D_menu").css("display", "none");
...
/*开始菜单查找*/
    $("#serInpu").on("input",function (e) {
        var inputValue = this.value;
        if(inputValue.trim() != "") {
            // console.log(inputValue)
            $("#programs").find("li").css("display", "none")
                .each(function (index, ele) {
                    if($(ele).children("a").text().trim() == "所有程序")
                        $(ele).css("display","block")
                            .children("a").css("display","none")
                            .next().css("display","block");
                    if($(ele).children().attr("id") == "serInpu")
                        $(ele).css("display","block");
                    // console.log(inputValue.trim())
                    // console.log($(ele).children("a").text().trim())
                    // console.log($(ele).children("a").text().trim().indexOf(inputValue.trim()))
                    // $(ele).children("a").text().trim().indexOf(inputValue.trim()) != -1
                    console.log(eval("/"+inputValue.trim()+"/i").test($(ele).children("a").text().trim()))
                    if (eval("/"+inputValue.trim()+"/i").test($(ele).children("a").text().trim()))
                        $(ele).css("display", "block")
                });
        }
        else{
            $("#programs").find("li").css("display", "none")
                .end().children("li").css("display","block");
            $("#allProgram").children("a").css("display","block")
                .next().css("display","none").children().css("display","block");
        }
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    })
/*/开始菜单查找*/
...
/*窗口文件事件*/
    function winFile(lm,bread,that) {
        var outClassName = $(that.id + "_main").children().eq(0).attr("class");
        var inClassName = $(that.id + "_main").children().eq(0).children().eq(0).attr("class");
        bread.attr("class",outClassName);
        bread.children().eq(0).attr("class",inClassName);
        bread.on("dblclick",function (e) {
            if($(this).children().children().attr("value") == "新建文本文档.txt"){
                console.log("new create text")
            }
            // alert(1)
            ...
            if(that)
            that.saveContainer.push($($(this).parent("div")[0]).children(".Q"));//用于回退或前进
            ...
            // console.log(that.id + " #Computer_back")
...
/*删除文件夹或其他文件*/
    $("#delete").on("click",function (e) {
        $('#comSelet_menu').data("target").style.display = "none";
        if(delSeq.length == 0) {
            $("#newOne").before('<li id="cancelDel"><a href="#">撤销&nbsp;删除(U)</a></li>');
            cancelFlag = true;//重新给撤销标记赋值，不然会蹦。。。
        }
        delSeq.push($('#comSelet_menu').data("target"));//添加到删除队列
        if(cancelFlag) {
            $("#cancelDel").on("click", function (e) {
                delSeq.pop().style.display = "block";
                Hidden_menu();
                if (delSeq.length == 0)
                    $(this).remove();
                e.stopPropagation ? e.stopPropagation() : e.cancelBubble = true;
        ...
        /*回收站添加删除项目*/
            $("#recycle_r").append($($('#comSelet_menu').data("target")).clone(true).css("display","block"));
            // console.log($($('#comSelet_menu').data("target")).clone(true))
        /*/回收站添加删除项目*/
        Hidden_menu();
...
/*窗口下文件查看in listStyle*/
    $(".listView").on("click",function (e) {
        var target = $(this).closest("[id$='Content']").children("[id$='main']");
        target.find(".Q").addClass("list-view-out")
            .children(".i").addClass("list-view-in").css({"top" : 0,"left" : 20});
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    })
/*/窗口下文件查看in listStyle*/
...
/*回收站清空回收站*/
    $(".recycle_Eliminate_all").on("click",function (e) {
        $("#recycle_r").children().css("display","none");
        delSeq.length = 0;//清空删除队列
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
        manuArr.push(this.dataset["code"]);
    })
/*/回收站清空回收站*/
...
/*回收站还原所有项目*/
    $(".recycle_reduction_all").on("click",function (e) {
        $("#recycle_r").children().css("display","none");
        while(delSeq.length > 0){
            $("#cancelDel").trigger("click");
            // console.log(1)
        }
        manuArr.push(this.dataset["code"]);
        // console.log(delSeq.length)
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    });
/*/回收站还原所有项目*/
...
/*复制粘贴*/
    $("#copy").on("click",function (e) {
        $(".paste").data("copyObj",$($('#comSelet_menu').data("target")).clone(true));
        console.log($($('#comSelet_menu').data("target")))
        Hidden_menu();
        manuArr.push(this.dataset["code"]);
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    });
    $(".paste").on("click",function (e) {
        // alert(1)
        console.log($(".paste").data("copyObj"))
        // console.log(this.dataset.copyObj)
        $('#computer_r_menu').data("parent").appendChild($(".paste").data("copyObj")[0]);
        manuArr.push(this.dataset["code"]);
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    })
/*/复制粘贴*/
...
/*右击打开*/
    $("#openItem").on("click",function (e) {
        $($('#comSelet_menu').data("target")).trigger("dblclick");
        Hidden_menu();
        manuArr.push(this.dataset["code"]);
        e.stopPropagation?e.stopPropagation() : e.cancelBubble = true;
    });
